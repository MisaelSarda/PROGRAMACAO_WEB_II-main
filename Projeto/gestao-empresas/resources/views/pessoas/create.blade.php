...
<div class="max-w-xl mx-auto bg-white p-6 mt-8 rounded shadow">
    ...
    <form action="{{ route('pessoas.store') }}" method="POST" class="space-y-4">
        ...
        <div>
            ...
            <input type="email" name="email" value="{{ old('email') }}" class="w-full border rounded px-3 py-2 shadow-sm">
            @error('email')<p class="text-red-500 text-sm mt-1">{{ $message }}</p>@enderror
        </div>

        <div>
            <label class="block text-sm font-medium text-gray-700">Telefone</label>
            <input type="text" name="telefone" value="{{ old('telefone') }}" class="w-full border rounded px-3 py-2 shadow-sm">
        ...
        <div>
            ...
            <textarea name="vulnerabilidade" rows="4" class="w-full border rounded px-3 py-2 shadow-sm">{{ old('vulnerabilidade') }}</textarea>
