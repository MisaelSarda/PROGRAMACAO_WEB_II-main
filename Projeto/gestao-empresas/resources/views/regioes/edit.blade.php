...
<div class="max-w-2xl mx-auto bg-white p-6 mt-8 rounded shadow">
    ...
    <form action="{{ route('documentos.update', $documento->id) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
        ...
        <div>
            ...
            <input type="date" name="validade" value="{{ old('validade', $documento->data_validade) }}" class="w-full border rounded px-3 py-2 shadow-sm">
            @error('validade')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            @if($documento->arquivo)
                <p class="text-sm mt-1">Arquivo atual: <a href="{{ asset('storage/' . $documento->arquivo) }}" target="_blank" class="text-blue-500 underline">Visualizar</a></p>
            @endif
            @error('arquivo')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        ...
        <div>
            ...
            <select name="regiao_id" class="w-full border rounded px-3 py-2 shadow-sm">
                @foreach($regioes as $regiao)
                    <option value="{{ $regiao->id }}" {{ old('regiao_id', $documento->regiao_id) == $regiao->id ? 'selected' : '' }}>
                        {{ $regiao->nome }}
                    </option>
                @endforeach
            </select>
